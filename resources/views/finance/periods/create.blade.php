<x-layouts.admin title="Tambah Periode">
    <x-slot:header>
        <div class="flex items-center justify-between">
            <div class="flex items-center gap-4">
                ...
                <h1 class="text-2xl font-bold text-gray-900 dark:text-white">Tambah Periode</h1>
            </div>
        </div>
    </x-slot:header>

    <x-ui.card>
        <form method="POST" action="{{ route('admin.finance.periods.store') }}" class="space-y-4">
            @csrf
            <x-ui.input name="name" label="Nama Periode" required help="Contoh: Januari 2026" />
            <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                <x-ui.input name="start_date" label="Start Date" type="date" required />
                <x-ui.input name="end_date" label="End Date" type="date" required />
            </div>
